...
import logging
import matplotlib
matplotlib.use("Agg")  # ใช้ backend ที่ไม่ต้อง GUI ประหยัด memory
...
# Cache ข้อมูลหุ้น 15 นาที (ลดการดึงข้อมูลซ้ำ)
_stock_data_cache = TTLCache(maxsize=500, ttl=900)  # 15 นาที
_benchmark_cache = TTLCache(maxsize=10, ttl=3600)   # 1 ชั่วโมง
_symbol_list_cache = TTLCache(maxsize=2, ttl=86400)  # 1 วัน

# ==========================================================
# OPTIMIZATION: Rate Limiting & Retry
# ==========================================================
_last_request_time = 0
_min_request_interval = 0.5  # รอขั้นต่ำ 0.5 วินาทีระหว่าง requests

def _rate_limited_fetch(fetch_func, *args, **kwargs):
    """Wrapper ที่มี rate limiting และ retry"""
...
# ==========================================================
# OPTIMIZATION: Unified Data Fetching
# ==========================================================
def fetch_stock_data(symbol: str, period: str = "3y", interval: str = "1d", force_refresh: bool = False):
    """
    ดึงข้อมูลหุ้นแบบ unified มี caching
    - ถ้าข้อมูลอยู่ใน cache -> ใช้เลย
    - ถ้าไม่มี -> ดึงใหม่แล้วเก็บ cache
    """
    cache_key = f"{symbol}_{period}_{interval}"

    if not force_refresh and cache_key in _stock_data_cache:
...
def fetch_benchmark_data(benchmark: str = "^GSPC", period: str = "3y", interval: str = "1wk"):
    """ดึงข้อมูล benchmark มี caching"""
    cache_key = f"benchmark_{benchmark}_{period}_{interval}"

    if cache_key in _benchmark_cache:
        return _benchmark_cache[cache_key].copy()

    data = _rate_limited_fetch(
        lambda: yf.Ticker(benchmark).history(period=period, interval=interval)
    )
...
def fetch_1m_return(symbol: str):
    """ดึง 1-month return จาก cache ถ้ามี"""
    cache_key = f"return_1m_{symbol}"
    if cache_key in _stock_data_cache:
        data = _stock_data_cache[cache_key]
    else:
        data = _rate_limited_fetch(
...
def calculate_mansfield_rs(symbol: str, benchmark: str = "^GSPC", ma_length: int = 52):
    # OPTIMIZED: ใช้ unified fetch มี caching
    stock = fetch_stock_data(symbol, period="3y", interval="1wk")
    index = fetch_benchmark_data(benchmark, period="3y", interval="1wk")

    if stock is None or index is None or stock.empty or index.empty:
...
def calculate_sata(symbol: str):
    # OPTIMIZED: ใช้ unified fetch มี caching
    stock = fetch_stock_data(symbol, period="3y", interval="1wk")
    benchmark = fetch_benchmark_data("^GSPC", period="3y", interval="1wk")
...
# Market Comparison
# ==========================================================
# OPTIMIZED: ใช้ caching ผ่าน fetch_1m_return
def one_month_return(symbol):
    return fetch_1m_return(symbol)
...
# ==========================================================
# OPTIMIZED: Core Analysis Pipeline (รวมการดึงข้อมูล)
# ==========================================================
def analyze(symbol: str) -> dict:
    # ดึงข้อมูลครั้งเดียว ใช้ caching
    data = fetch_stock_data(symbol, period="3y")
    ...
    # OPTIMIZED: ดึง benchmark ผ่าน caching
    return {
        "price": price,
        ...
        "stock_1m": fetch_1m_return(symbol),
        "nasdaq_1m": fetch_1m_return("^IXIC"),
        "sp500_1m": fetch_1m_return("^GSPC"),
    }
...
# ==========================================================
# OPTIMIZED: Technical Chart (ใช้ข้อมูลจาก cache)
# ==========================================================
def plot_technical_chart(symbol: str):
    apply_tv_style()

    # ดึงข้อมูลจาก cache (ไม่ต้องดึงใหม่!)
    data_3y = fetch_stock_data(symbol, period="3y")

    if data_3y is None or data_3y.empty or len(data_3y) < 50:
        raise ValueError("NOT_ENOUGH_DATA")
    ...
    legend1.get_frame().set_facecolor("#1c2128")
    legend1.get_frame().set_edgecolor("#2a2e39")
    ...
    buf = io.BytesIO()
    plt.tight_layout()
    plt.savefig(buf, format="png", dpi=100)  # ลด DPI เพื่อประหยัด memory
    plt.close(fig)  # ปิด fig เพื่อล้าง memory
...
# ==========================================================
# OPTIMIZED: Symbol Fetching with Caching
# ==========================================================
def get_sp500_symbols():
    cache_key = "sp500_symbols"
    if cache_key in _symbol_list_cache:
    ...
    try:
        ...
        _symbol_list_cache[cache_key] = symbols
        return symbols
    except Exception as e:
        logging.error(f"Failed to fetch S&P500 symbols: {e}")
        return []
...
def get_nasdaq100_symbols():
    ...
    try:
        ...
        logging.error(f"Failed to fetch NASDAQ100 symbols: {e}")
        return []

def get_all_symbols():
    """
    รวมสองตลาด + ลบตัวซ้ำ (มี caching)
    """
...
def scan_impulse_green_streak(
    ...
    # OPTIMIZED: ใช้ ThreadPool สำหรับ concurrent fetching
    max_workers = min(10, len(symbols))  # จำกัด concurrent requests

    def process_symbol(symbol):
        try:
            data = fetch_stock_data(symbol, period=f"{lookback_months}mo")
...
# ==========================================================
# OPTIMIZED: Stage Scan Engine (Concurrent)
# ==========================================================
...
async def cmd_sr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # OPTIMIZED: ใช้ cache แทนการดึงใหม่
        data_1y = fetch_stock_data(symbol, period="1y")
        if data_1y is None or data_1y.empty or len(data_1y) < 50:
            raise ValueError
    except Exception:
...
# ==========================================================
def main():
    logging.info("Pro Investor AI Stock Bot Started (OPTIMIZED)")
